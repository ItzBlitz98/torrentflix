    ...
    function selectTorrent(data) {
      inquirer.prompt([
        ...
        if(answer.torrent === "b"){
          ...
          } else if(data[number].torrent_project_hash){
            var hash = data[number].torrent_project_hash;
            torrentproject.get_link(hash).then(
              function (data) {
                var magnet_link = "magnet:?xt=urn:btih:" + hash + "&dn=" + torrent_title + "&tr=" + data.join("&tr=");
                console.log('Streaming ' + chalk.green(torrent_title));
                parseTorrent(magnet_link, torrent_title);
              }, function (err) {
                  console.log(chalk.red(err));
                  firstPrompt(options);
              });
