...
var config_object = require('../torrentflix_config.json');
...
function main() {
    ...
    inquirer.prompt([
      ...
      if(answer.torrent === "b"){
        ...
        if(data[number].torrent_link){
          ...
          console.log('Saving ' + chalk.green.bold(torrent_title));
        ...
        inquirer.prompt([
          {
            ...
            if(answer.torrent === "b"){
              ...
              console.log(chalk.magenta.bold(torrent_title));
              fs.writeFile(config.download_location, torrent, function(err){
                  if(err) console.log(printError('Error writing torrent file'));

                  console.log(chalk.magenta.bold(torrent_title +' saved'));
              });
              //console.log('Streaming ' + chalk.green.bold(torrent_title));
  ...
  function writeHistory(torrent_name) {
    var last_history = history_object[history_object.length-1];

    if(last_history.torrent !== torrent_name){
      data_content = {
  ...
  config_location = './torrentflix_config.json';
  history_location = './torrentflix_history.json';
