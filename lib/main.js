(function() {
    ...
    exports.AppInitialize = start = function() {
    ...
    function printHistory(){
      if(history === "true"){
        ...
        if(get_hist_object.size < 1){
          console.log(chalk.bold.red("Your history file is empty, Go watch some torrents!"));
        } else {
          console.log(chalk.green.bold("Watched History: "));
          Object.keys(all_hist).forEach(function(key) {
              histcount = 1;
              console.log(chalk.red.bold(histcount + ' ') + chalk.magenta.bold(all_hist[key]));
              histcount++;
          });
        }
