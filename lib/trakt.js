                                    ...
                                    case "Trending":
                                        trakt.movies.trending({}).then(trendingMovies => {
                                            return Promise.all(trendingMovies.map(element => {
                                                return element.movie.title + " " + element.movie.year;
                                            })).then(trendingMoviesList => {
                                                trendingMoviesList.push('<< Exit >>');
                                                inquirer.prompt([{
                                                    type: "list",
                                                    name: "TraktTrendingMovies",
                                                    message: chalk.green("Trending Movies on Trakt"),
                                                    pageSize: 15,
                                                    choices: trendingMoviesList
                                                }]).then(answer => {
                                                    if (answer.TraktTrendingMovies == '<< Exit >>') return;
                                                    main.AppInitialize({search: answer.TraktTrendingMovies});
                                                }).catch(chalk.red(console.log));
                                            })
                                        }).catch(chalk.red(console.log));
                                        break;
                                            ...
                                            trakt.shows.trending({}).then(trendingShows => {
                                                return Promise.all(trendingShows.map(element => {
                                                    ...
                                                    inquirer.prompt([{
                                                        ...
                                                        //Get index of trendingshowlist that match the answer. then take the last bit of it and paste it in id:
                                                        for (var i = 0; i < trendingShowsList.length; i++) {
                                                        ...
                                                        //main.AppInitialize({search: answer.TraktTrendingShows});
                                                    }).catch(chalk.red(console.log));
                                                })
                                            }).catch(chalk.red(console.log));
                                            break;
