  ...
  search: function(query, rarbg_api_url, cat, page, limit) {
    ...
    request(url, function (err, response, body) {
        if (!err && response.statusCode === 200) {
            ...
            var search_url = rarbg_api_url + "/pubapi_v2.php?mode=search&search_string=" + encodeURIComponent(search_query) + "&sort=seeders&format=json_extended&token=" + token;

            request(search_url, function (err, response, body) {
                if (!err && response.statusCode === 200) {

                    data = JSON.parse(body);
                     if(data.torrent_results.length > 1){
