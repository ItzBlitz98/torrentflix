(function () {
    ...
    if (program.config) {
        if (program.args[0]) {
            ...
            if (conf.size < 1) {
              console.log(chalk.bold.red('A config file has not been created yet.'));
              console.log(chalk.bold.red('Please run torrentflix at least once to create one.'));
            } else {
        ...
        request(url, function (err, response, body) {
            if (!err && response.statusCode === 200) {
                ...
                if (data.version > pkg.version) {
                    console.log(chalk.bold.red('A new version of torrentflix is available.'));
                    console.log(chalk.bold.red('Run "npm install -g torrentflix'));
                    console.log(chalk.bold.red('Or go here to get it: https://github.com/ItzBlitz98/torrentflix'));
                }
