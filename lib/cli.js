(function () {
    ...
    function AppInitialize(prog) {
        ...
        if (conf.size < 1) {
            ...
            for (var source_name in torrent_sources) {
                conf.set(source_name, torrent_sources[source_name]);
            }
            //add config options
            for (var option_name in conf_options) {
                conf.set(option_name, conf_options[option_name]);
            }
